...
class EngineManager(private val context: Context, private val pickToken: String? = null) {
  ...
  // 4Debian 双运行时（design-4debian D-0/D-6）：main = Termux 树（usr/bin/node）；
  // debian = Debian rootfs 整树挂 files/usr（usr/usr/bin/node…，usr/usr/lib/aarch64-linux-gnu/
  // 下是 glibc ld.so），启动链从「直接 exec node」换成「exec ld.so + [ld.so, node, …]」。
  private val debianRuntime = BuildConfig.DEBIAN_RUNTIME
  private val nodeBin = if (debianRuntime) File(usrDir, "usr/bin/node") else File(usrDir, "bin/node")
  private val dshBin =
    if (debianRuntime) File(usrDir, "usr/lib/node_modules/@deepseek-ai/dsh/lib/bin.js")
    else File(usrDir, "lib/node_modules/@deepseek-ai/dsh/lib/bin.js")
  // D-6 直启链三件套（坑 97/98/102）：ld.so 自足 ELF（无 PT_INTERP，内核可直接 exec）；
  // LD_PRELOAD 拦截器让引擎 fork+exec 的 guest 工具链（git/rg/bash/shebang）递归包装。
  private val d6Ldso = File(usrDir, "usr/lib/aarch64-linux-gnu/ld-linux-aarch64.so.1")
  private val d6Hook = File(usrDir, "opt/d6/libd6exec.so")
  /**
   * D-6 argv 包装（坑 97/98）：main 返回原样；debian 把 guest glibc ELF 的 exec 改写为
   * [ldso, --argv0, prog, prog, args…]（ld.so 无 PT_INTERP，内核可直接 exec）。ConsoleSession
   * （bash -i）与 UndoGate（node undo-emergency.mjs）共用。
   */
  fun d6Wrap(prog: File, args: List<String>): List<String> =
    if (debianRuntime) listOf(d6Ldso.absolutePath, "--argv0", prog.absolutePath, prog.absolutePath) + args
    else listOf(prog.absolutePath) + args

  /** 相对 usrDir 的 node/bash 路径（ConsoleSession/UndoGate/UpdateManager 等 flavor 无关复用） */
  val nodeRelPath: String = if (debianRuntime) "usr/bin/node" else "bin/node"
  val bashRelPath: String = if (debianRuntime) "usr/bin/bash" else "bin/bash"
  ...
  private fun bundledFingerprint(): String = try {
    context.assets.open(if (debianRuntime) "debian-rootfs.sha256" else "snapshot.sha256")
  ...
  private fun stagedRuntimeComplete(stage: File): Boolean {
    // 布局契约（M1）：tar 顶层 = usr/(=Debian rootfs 整树) + home/。stage 解出后
    // Debian 绝对路径 /X 落在 stage/usr/X。main 探针 = stage/usr/bin|lib/…；
    // debian 探针 = stage/usr/usr/bin/node、stage/usr/usr/lib/.../bin.js、stage/usr/opt/d6/…。
    val node = if (debianRuntime) File(stage, "usr/usr/bin/node") else File(stage, "usr/bin/node")
    val bin =
      if (debianRuntime) File(stage, "usr/usr/lib/node_modules/@deepseek-ai/dsh/lib/bin.js")
      else File(stage, "usr/lib/node_modules/@deepseek-ai/dsh/lib/bin.js")
    val profile = File(stage, "home/.dsh/profiles/web")
    if (debianRuntime && !SnapshotFs.exists(File(stage, "usr/opt/d6/libd6exec.so"))) {
      Log.e(TAG, "staged runtime incomplete: d6 exec hook missing")
      return false
    }
  ...
  private fun extractSnapshotTo(stage: File, onProgress: (Long, Long) -> Unit): Boolean {
    return try {
      val asset = if (debianRuntime) "debian-rootfs.tar.xz" else "snapshot.tar.xz"
      val fd = context.assets.openFd(asset)
      SnapshotExtractor.extract(
        context.assets.open(asset), fd.length, stage, onProgress, runtimeRoot = context.filesDir,
      )
      true
    } catch (t: Throwable) {
  ...
  fun startEngine(port: Int = EngineProbe.ENGINE_PORT, force: Boolean = false): Boolean {
    ...
    // 4Debian M1 收养闸：健康引擎在听（200/401/303 均算活着）且本代无句柄时直接收养——
    // 绝不允许后续 ensure/看门狗重入走 killExistingEngine（pkill -f bin.js）把健康引擎杀掉
    // （模拟器实锤：3 路 startEngine 在 46s 内互相残杀，engine.log 全缓冲 0 字节、SIGTERM 无痕）。
    if (!force && engineProcess?.isAlive != true) {
      try {
        if (EngineProbe.check(1_500).optBoolean("running", false)) {
          LogCollector.log(TAG, "engine start: adopted an already-listening engine (port " + port + ")")
          return true
        }
      } catch (_: Throwable) {
      }
    ...
    // 4Debian：拦截器换 D-6 glibc 钩子（usr/opt/d6/libd6exec.so），闸门口径随 flavor。
    val preload = if (debianRuntime) d6Hook else File(usrDir, "lib/libtermux-exec-ld-preload.so")
    if (!preload.exists()) {
      Log.e(TAG, "engine start failed: exec-interceptor preload missing at " + preload.absolutePath)
      return false
    }
    ...
    return try {
      ...
      if (!debianRuntime) applyRuntimePatches() // Termux 专属运行时补丁面；Debian 树不适用
      ...
      val args = if (debianRuntime) {
        // D-6 直启链（定稿）：exec 自足 ld.so，argv = [ldso, --argv0, node, node, …]。
        // 坑 104 三连排除：PTY（script -qec）被 app 域 SELinux 拒（/dev/ptmx，rc=2）；
        // stdbuf 找不到 libstdbuf.so（裸名搜索在宿主命名空间无解）；node 对文件描述符
        // 本就同步直写（libuv 非 TTY 路径），token 行无需额外缓冲处理——此前「不落盘」
        // 的真凶是 EADDRINUSE（坑 100）与 script-rc2 速死，不是缓冲。
        arrayOf(
          d6Ldso.absolutePath, "--argv0", nodeBin.absolutePath, nodeBin.absolutePath,
          "--expose-internals", dshBin.absolutePath, "web", "--port", port.toString(), "--no-open",
        )
      } else {
        arrayOf(
  ...
  private fun killExistingEngine() {
    ...
    LogCollector.log(TAG, "killExistingEngine: engine port still occupied after cleanup (release recheck failed)")
  }
  ...
  fun shellEnv(): Map<String, String> {
    if (debianRuntime) {
      // 4Debian D-6 环境（坑 98/99/101/102 实证链）：证书走 Debian 布局；exec 拦截 =
      // glibc 版 libd6exec.so（Termux 的 bionic 钩子对 glibc tracee 无效）；TMPDIR 进 rootfs。
      // PATH 把 guest usr/bin 放前（引擎子进程命中 guest 工具链）；宿主工具由钩子的
      // guest 优先重映射兜底。
      val ca = File(usrDir, "etc/ssl/certs/ca-certificates.crt")
      return mapOf(
        "PATH" to (usrDir.absolutePath + "/usr/bin:/system/bin"),
        "LD_LIBRARY_PATH" to File(usrDir, "usr/lib/aarch64-linux-gnu").absolutePath,
        "HOME" to homeDir.absolutePath,
        "DSH_HOME" to ensurePrivateDshData().absolutePath,
        "DSH_FILES_DIR" to context.filesDir.absolutePath,
        "TMPDIR" to File(usrDir, "tmp").apply { mkdirs() }.absolutePath,
        "LD_PRELOAD" to d6Hook.absolutePath,
        "D6_LDSO" to d6Ldso.absolutePath,
        "D6_ROOT" to usrDir.absolutePath,
        // 坑 104（seccomp）：app 进程（zygote 系）带 bionic seccomp 白名单，glibc 2.36 启动即
        // rseq 注册撞白名单外 syscall → SIGSYS(exit 159) 150ms 静默死；run-as/shell 域无此
        // 过滤器故手动链全通。官方开关关闭 glibc rseq 注册。
        "GLIBC_TUNABLES" to "glibc.pthread.rseq=0",
        "SSL_CERT_FILE" to ca.absolutePath,
        "CURL_CA_BUNDLE" to ca.absolutePath,
        "GIT_SSL_CAINFO" to ca.absolutePath,
        ...
        "TERMUX_APP__LEGACY_DATA_DIR" to "/data/data/com.dsharnessmobile.shell4d",
      )
    }
