...
| **P2** 壳侧集成 | 解压契约、启动链、DNS、绑定表、控制台、探针 | **进行中**：**引擎 overlay 升级 0.1.5-rc.1 接线完成（2026-09-13）**——`scripts/build-rootfs-g2.mjs` 一键固化 G2 载荷（LFS 基座自动拉取 + 312 包登记表覆盖 + rootPackage 换 lib + koffi 平台包双保险 + 版本/keepUnpublished 断言 + 归档），模拟器全量回归 ALL PASS（token 303 兑换鉴权流探针，坑 101）；**与现网 Termux 快照引擎版本对齐**。**D-6 启动器原型✅（坑 102）**：libd6exec.so（LD_PRELOAD exec 族+posix_spawn 拦截 + /bin、/usr guest 优先重映射 + shebang 递归判定；A/B 对照 5/5↔5/5）。待：G3 完整版、解压契约、DNS、绑定表、探针门控、壳侧变体 App（改名共存，部署目标） |
