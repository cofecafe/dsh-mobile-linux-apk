...
> 版本 v0.1（P0）｜ 2026-09-13 ｜ 分支 `4Debian`（未合入 main）｜ 状态：**方向已决策**（2026-09-13 用户 Discord 确认选项 A），实施未开始。
...
- **R-7【低】构建环境**：debootstrap 跨 arch 需 qemu-user-static/binfmt（WSL2 可做，沿用 build-snapshot-013 的 WSL 内执行模式）；GHA 免费 arm64 runner 仅 public 仓库——本仓镜像面可承担，协调仓私有侧 qemu 兜底。
