...
105. **客户端模块表三连坑（P2 实锤，2026-09-14 模拟器）**：Web UI「Failed to load plugins」的三个独立死因，一个都不能少——
    ① **inject 目标必须是 Loader entry**：`dsh.client.inject` 的包名会被浏览器端 `arriveGraphRow` 按 graphRows 查找，宿主（dsh-client-modules）只为「声明 dsh.client 的 cordis Loader entry」出 wire 行，**inject 列表从不自动展开成行**——目标包缺席 = 浏览器 require 时 `missed the module table`。patch 层 entry 又是从 **profile 目录**解析的（bundle 层才从引擎树解析），@deepseek-ai 包名在 profiles/web/node_modules 物化（或符号链接回引擎树，node realpath 语义让依赖在引擎树内解析）才能成行。
    ② **双 provider 撞车**：`dsh-client-runtime`（provide sessions/workspaces）与 `dsh-api-session-controller`/`dsh-api-workspace-controller`（同样 provide）**是两代同位服务**——同时成 entry = `service "sessions" has been registered at <…>` 恒炸、两条 controller fiber 双双Rejected、整页 Failed to load plugins。上游桌面组合=只有 controllers（web-app bundle 无 runtime 行）；`@dsh-android/dsh-client-ui-responsive` 0.3.3 只把 runtime 当**模块**用（stores.js 的 `defineStore` import），**不**需要它当插件——所以 profile 的 cordis.patch.yml **永远不要**给 client-runtime 写 insert（v0.14.0-preview 快照的 yml 模板本来就没有；谁写谁炸）。
    ③ **设备侧 yml 是用户态文件、跨装机保留**：事务化换树只换工厂件（usr 树 + 出厂 home 的**新增**），**不回滚用户已改的 cordis.patch.yml**——早期实验写入的 `client-runtime` insert 被引擎 patchReload=live 吸收成规范形（注释剥掉、130 行）后**每次装机都活着**，看着像「引擎自动注入」，其实是自己埋的雷。修法=设备侧删该 insert（坑19 冷启）或 pm clear 净装。**取证工具**：CDP（`adb forward tcp:X localabstract:webview_devtools_remote_<pid>`，WS 路径 `/devtools/page/<id>`，响应取值要三层 `.result.result.value`）钩 `Promise.allSettled` 抓 fiber 拒因——console 里只有 AggregateError 一行，真错在每条 promise 的 rejection 里。
106. **@dsh-android 插件的 3080 硬编码（P2.5，Debian flavor 专属）**：`dsh-android-file-open/lib/route-auth.js` 的 `TRUSTED_HOSTS=['127.0.0.1:3080','localhost:3080']` 与 `dsh-android-browser` 同款——Termux flavor 端口 3080 无感，Debian flavor 引擎在 **3081**（与 vivo 原应用共存让端口），来件三条 exact 路由 Host 白名单恒不中 → **403 空体风暴**（WebView 每 4s 一条 `[dsh-mobile] file-incoming unauthorized`，上游 webserver 的 exact-先于-prefix 匹配使插件自带鉴权成为唯一闸门，见坑78）。修法=build-rootfs-g2 步骤① sed `:3080→:3081`（file-open 的 js+d.ts、browser 的 index.js）+ 残留 3080 门禁拒出货；断言=净装后 logcat 的 file-incoming 计数归零。**顺带**：`dsh-web-frontend` 的 asset hash 与引擎树一致不代表客户端栈无恙——UI 死法全在 fiber 激活期，只有页面级证据（截图/CDP 拒因）算数。

107. **flock-F3 的平台包暗缺（坑107，G3 会话烟测实锤，2026-09-14）**：引擎树 md5 对账只比 JS——**原生平台包不在对账面**。`@deepseek-ai/node-addon-system` 的 optionalDependencies（`-linux-arm64/-x64` 等）在 npm 语义下由安装平台决定，g2 的 overlay 登记表不含 optional 依赖、基座树同缺 → **F3 补丁后的 flock.js 在 linux 上 `require.resolve('@deepseek-ai/node-addon-system-linux-arm64/package.json')` 直接 MODULE_NOT_FOUND**，症状极具迷惑性：欢迎页/会话创建全正常，**首条消息发出即 "This turn failed: Cannot find module …"**（会话持久化要 flock）。上游 v0.14.0-preview 快照同样缺包（`tar -tf | grep node-addon-system-linux` 零命中）——preview 自身的会话回合也是瘸的。修法=`build-rootfs-g2.mjs` **④b 步**：版本锁 overlay 缓存里 node-addon-system@0.1.2 的 optionalDependencies（镜像 koffi ④ 的双保险模式），包内自带 `bin/glibc/system.node`+`bin/landlock-run` 装到 `$NM/@deepseek-ai/dsh/node_modules/@deepseek-ai/` 嵌套位（flock.js 从 lib/ 向上解析正好命中）。**验收=发一条消息看回合走到模型调用**（TRANSPORT/API 错误=配置缺 key，属预期终态；Cannot find module=没修好）。**顺带**：execFileSync tar 读包内文件必须 `-xzOf` 带 **O**（缺 O 是写盘不是 stdout，JSON.parse 吃空串炸得莫名其妙）；无 key 环境的会话烟测照样能验：会话建立/消息气泡/上下文注入（system-prompt+skill-catalog）/重试计数（5/5）/错误分类（TRANSPORT）全链 UI。

108. **构建容器 DNS 泄漏 + macOS 构建沙箱三连（2026-09-14 vivo 实测）**：
    ① **resolv.conf 烤包**：g2 在 Docker 容器里归档 rootfs，容器的 `/etc/resolv.conf`（Docker 内部 DNS，如 `nameserver 0.250.250.200`——0.x 段根本不可路由）被原样收进 tarball → 真机 guest glibc 全量出站 DNS 解析失败——**症状面极大**：模型 API（deepseek/z.ai）、插件市场、任何 fetch 全报 transport failed，且与「没配 key」的错误长得一模一样。修法=g2 ①b 步：归档前 `printf` 公网 DNS（223.5.5.5/119.29.29.29/8.8.8.8）覆盖 + 残留 0.x 门禁。**验收必须看会话回合是否走到 HTTP 层**（401/超限），纯 transport ≠ 修好。
    ② **optionalDependencies 平台包是系统性暗缺**（坑107 的推广）：npm 平台 optional 包（node-addon-system-\*、@napi-rs/canvas-linux-\*-gnu、koffi-\*）在「登记表覆盖 + 基座树」双源构建里两头都没有——**JS md5 对账永远看不见**。已有 ④（koffi）/④b（node-addon-system）/④d（canvas）三例，新引擎包凡带 `optionalDependencies` 平台包的都要进 g2 补丁位。症状分布：flock=会话首回合炸、canvas=开机警告+图片渲染废。
    ③ **macOS 沙箱下 gradle 静默挂死**：Gradle 8.11 的 macOS FSEvents 文件监视器（`OsxFileWatcher.startWatching0` 原生调用）在受限环境里**永不返回**——构建在第一个真实任务后整体冻结、零日志、零错误（jstack 抓 worker RUNNABLE 卡在 native 才破案）。修法=`-Dorg.gradle.vfs.watch=false`（回退轮询）。顺带：被 SIGTERM 杀过的构建会留 wrapper `.lck`/VFS 锁（清 `.gradle` + `caches/**/*.lock`）；`ps` 在收紧后的沙箱里直接 Operation not permitted——**判断进程死活别信 ps，用 `jps` 或看产物 mtime**；adb 装大包失败报错为空=流式安装通道断，改 `push` 到 `/data/local/tmp` + `pm install`。

109. **guest DNS 三层洋葱（2026-09-14 vivo 实战，最难一坑）**——「连不上模型/z.ai/插件市场，TRANSPORT failed」的完整解剖，每层修完都像好了其实只是露出下一层：
    ① **resolv.conf 内容毒**（坑108①）：构建容器 Docker 内部 DNS 烤进 rootfs——修了才知道 guest 根本不读这份文件（必要不充分）。
    ② **读路径断裂（真根因）**：引擎不 chroot，glibc 解析器读**安卓真根** `/etc/resolv.conf`——安卓没有这文件（bionic 走 netd）→ glibc 兜底 `nameserver 127.0.0.1` → 本机 53 无人听 → **一切外网名字 EAI_AGAIN**。表象与「没配 key」的错误一模一样（fetch 都报 transport failed）。/etc 只读 + app 无 CAP_SYS_ADMIN，修不动根；LD_PRELOAD 拦 open/openat 也没用（glibc 内部 __open_nocall 走隐藏符号不过 PLT）。
    ③ **修法的两个反坑**：(a) getaddrinfo 拦截必须**混合策略**——先走真 getaddrinfo（localhost 等 /etc/hosts 名零回归），败走自建 UDP DNS；直接全接管会把 `localhost` 也 DNS 化（NXDOMAIN）→ 引擎自连链路死 → Web UI 模块表停摆、输入无响应。(b) **freeaddrinfo 不能拦**——混合策略下 glibc 自家的结果链（混合分配）会进你的逐块 free → `free(): invalid pointer` 崩溃；反之 glibc 原生 freeaddrinfo 逐块 free 与自建 calloc 链完全兼容。另：自建 DNS 报文头是 **12 字节**（qd 后还有 an/ns/ar 三个 count 共 6 字节，只写 4 个=恒 FORMERR，服务器 12 字节应答、wireshark 都不用上，`recv r=32 rcode!=0` 即是）。
    **取证武器**：同环境链手工 node 双测——`fetch("http://223.5.5.5/")`（IP 直连，绕 DNS）vs `fetch("https://api.deepseek.com/")`（要解析）——`IPOK 404 + NAMEERR EAI_AGAIN` 一击定位是 DNS 而非网络/权限/TLS。**验收**：`localhost=127.0.0.1` + `fetch(api)=401`（真 API 应答）双绿才算修好；TRANSPORT failed 复现=没修。

110. **用户实测五连坑批（2026-09-16 vivo 反馈，坑1-5 对应消化）**——shell4d 当运行环境装 codex 的完整实战，暴露钩子/配置/沙箱三层缺陷，两条已构建侧根治：
    ① **LD_PRELOAD glibc 库毒死 bionic 子进程（钩子层真缺陷，已修）**：钩子靠 LD_PRELOAD 传播，但它是 glibc 构建——exec bionic 程序（/system/bin/sh、curl…PT_INTERP=/system/bin/linker64）时 bionic 链接器直接 "CANNOT LINK EXECUTABLE: libc.so.6 not found"。修法=d6 钩子原样 exec 出口统一消毒：目标（含 shebang 解释器）PT_INTERP 含 "linker" → envp 摘 LD_PRELOAD 再交内核；覆盖 execve/execv/execvp/execvpe/posix_spawn{,p}（execl 族内部转 execve）。静态/musl 无 PT_INTERP 不受影响。验证三绿：`/system/bin/sh` BIONIC-OK + glibc 子进程 exec OK + DNS 401 不回归。用户侧插件补丁（LD_PRELOAD='' 栅栏）为修复前的等效绕法。
    ② **prefix 指单层 usr（顶层孤儿区）**：上游快照 yml 的 prefix=files/usr——顶层 bin 只有 Termux 时代孤儿 bash（缺 libtinfo 跑不动，用户踩「libtinfo.so.6 not found」）；**完整工具链在双层 usr**（files/usr/usr/bin/bash + libtinfo，实测 GUEST-BASH-OK）。修法=g2 ①c 步 sed prefix 双 usr 化 + 单层门禁（exit 9）+ 旧包名门禁（exit 10，坑1 跨应用 uid 私有目录互不可见）。bashPath 上游已双 usr；home/cwd=files/home 正确（事务 livePath 把 rootfs 的 home/ 条目落位 files/home，与 EngineManager HOME 一致；files/usr/home/.dsh 是旧布局残渣）。
