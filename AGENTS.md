...
| 2026-09-16 | **4Debian 用户实测五连坑消化：坑110 钩子 bionic 消毒 + g2 ①c prefix 双 usr 对齐** | 用户以 shell4d 为环境装 codex 的实战反馈全消化：① **d6 钩子 LD_PRELOAD(glibc 构建) 毒死 bionic 子进程**（CANNOT LINK libc.so.6）→ 钩子原样 exec 出口统一消毒（PT_INTERP 含 linker 含 shebang 解释器 → envp 摘 LD_PRELOAD，覆盖 execve/execv/execvp/execvpe/posix_spawn 族），验证三绿（sh BIONIC-OK/glibc 子进程 OK/DNS 401 不回归），已热推 vivo；② 上游 yml prefix 指单层 usr（顶层孤儿 bash 缺 libtinfo，用户踩「libtinfo not found」）→ g2 ①c：sed 双 usr + 单层门禁 exit9 + 旧包名门禁 exit10（Docker GNU sed 实测 PASS），**双 usr bash+libtinfo 实测 GUEST-BASH-OK**；③ DNS 与坑109 互证（Android 禁应用绑 <1024 实锤、CONNECT 代理+自建 UDP 路线、qname 漏 \x00 根标签同类坑）；④ node -e/npm SIGSYS=坑104 未覆盖面，**musl 静态二进制=app 域第三方工具通用范式**；⑤ write 工具 EACCES（fs 禁 link→heredoc）等小坑 | AI 开发助手 |
