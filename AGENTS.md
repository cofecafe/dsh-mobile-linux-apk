...
| 2026-09-16 | **4Debian 坑111：pnpm 缺位插件全灭——g2 ④e 步出厂自带（pnpm 10.14.0）** | 症状「pnpm not found on PATH」装不了任何插件。根因=上游快照 profile 本就是 pnpm 布局而 Debian 基座只带 npm/corepack（Termux 有、Debian 换血丢）。修法=g2 ④e：缓存锁 tgz 解 lib/node_modules + bin 软链 + exit11 断言；**shebang 反坑**：原生 `#!/usr/bin/env node` 是解释器+单参数模式（d6_shebang_interp 不支持）→ 改 `#!/usr/bin/node` 走钩子重映射。vivo 实锤 spawn 链 `pnpm --version`=10.14.0 ✓，新 APK（222MB clean 版）已装机。伴生：gradle 增量打包 APK 尾部膨胀（232MB 内容显 459MB）clean 即消 | AI 开发助手 |
