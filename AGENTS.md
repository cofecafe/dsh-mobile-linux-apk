...
| 2026-09-13 | **4Debian G2 达成：dsh 引擎在 Android 上以 D-6 直启链跑起（P1 四门禁 G1/G2/G3-lite 全绿）** | **引擎树嫁接配方**：仓内 LFS 基座（`base/base-dsh.tar.xz` home/.dsh 纯 JS + `base/base-usr-arm64.tar.xz` 引擎本体）经 `media.githubusercontent.com` 直拉（本机无 git-lfs，sha256 对指针校验）嫁接进 Debian arm64 rootfs；**koffi 空壳目录坑（坑 99）**：树内 `build/koffi/linux-arm64/` 是空摆设，koffi v3 解析走 `@koromix/koffi-<plat>-<arch>` 平台包 → 下划线 triplet，修法 = `npm pack @koromix/koffi-linux-arm64@3.1.5`（精确锁版）双保险放置；node-pty 树内自带 linux-arm64 glibc prebuild 未挡路。**验收**：容器级 boot+HTTP200 → 模拟器级（Android 14 ARM64，无 proot 纯 ld.so 包装链）`dsh web @3080` 60s 就绪，设备端 node-fetch **HTTP 200 text/html** + adb forward 宿主 curl **HTTP 200** + Web UI HTML 全量返回（`scripts/g2-emulator.sh`，948MB 载荷 1.2s 推送）；env 口径复刻 EngineManager.shellEnv()（DSH_HOME/证书/TMPDIR，Debian 布局）。@napi-rs/canvas 缺失仅告警不挡 boot。基座引擎 0.1.1-rc.2（证明用），overlay 升级 0.1.5-rc.1 为 P2 接线 | AI 开发助手 |
